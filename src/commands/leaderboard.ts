...
module.exports = {
	...
	async execute(interaction: BaseCommandInteraction, newState: { channelId: any; member: { id: any; }; guild:{ id: any; };}) {
        ...
        try {
          const virgin = await orm.em.find(Virgin, 
            { guild: { $eq: guildId }, },
          );
          let response = "Leaderboard:"+ '\n';
          for(let i = 0; i < virgin.length; i++){
            response = response + virgin[i].username + ": " + virgin[i].virginity.toString();
